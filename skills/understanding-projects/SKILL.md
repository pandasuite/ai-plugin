...
- **Elements** — what the user sees as one element is often several nested units: a **Text**, an **Image**, a **Group**, an **Interactive Area**, or a **Multi-state**. Appearance, data, and actions can live on different units of that tree, so read the hierarchy before saying which element does what.
- **Properties** — many visible and behavioral settings live on properties. Depending on the property, its value can be fixed, **bound** to live data from a source (a **Datastore**, or another component's exposed data) that refreshes when the source changes, **overridden per state**, or **translated per language**. Per-state overrides record only what changes: a property's own value is its setting in the original state, and the other states inherit it. Bindings, states and languages can all affect the same property surface.
